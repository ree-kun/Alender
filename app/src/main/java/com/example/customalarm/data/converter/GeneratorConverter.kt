...
class GeneratorConverter {
    ...
    fun toGenerator(string: String): CalendarTargetIdentifierGenerator {
        ...
        return when (RepeatUnit.of(values[0])!!) {
            ...
            DAILY -> DailyIdentifierGenerator(DailyPitch(values[1].toInt()))
            WEEKLY -> WeeklyIdentifierGenerator(
                WeeklyPitch(values[1].toInt()), values.drop(2).mapNotNull { Day.of(it) })
            MONTHLY_DAY -> {
            ...
            MONTHLY_NTH_DAY -> {
                ...
                MonthlyWeekIdentifierGenerator(
                    remain.mapNotNull { it.toIntOrNull() }.map { NthDay(it) },
